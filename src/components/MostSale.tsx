...
import {mostsaleItems} from '../constants';

interface MostSaleProps {
  onScreen?: boolean;
}

interface MostSaleCardsProps {
  onScreen?: boolean;
  label: string;
  price: string;
  src: any;
...
const MostSale: FC<MostSaleProps> = ({onScreen}) => {
  const navigation = useNavigation<any>();
  return (
    <View style={styles.container}>
      <View style={onScreen ? {display: 'none'} : styles.textContainer}>
        <Text style={styles.textHeadtitle}>Most Sale</Text>
        <Text
          testID="trending-nav"
          style={styles.textSubtitle}
          onPress={() => navigation.navigate('MostSale')}>
          Show all
        </Text>
      </View>
      <FlatList
...
const MostSaleCard: FC<MostSaleCardsProps> = ({
  label,
  price,
  src,
  onScreen,
